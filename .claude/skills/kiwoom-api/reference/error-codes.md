...
응답 바디의 `return_code` 가 0이 아니면 `return_msg` 와 함께 아래 코드가 온다.
...
- `15xx` 요청 형식/필수값/헤더 오류 → 요청 조립 버그
- `1687` 재귀 호출 제한, `170x` 유량(요청 개수) 초과 → 호출 빈도 제어 필요
- `19xx` 종목/시장 코드 오류
- `80xx` 앱키·시크릿·토큰 문제 (`8005` 토큰 무효, `8010` 토큰 발급 IP 불일치)
