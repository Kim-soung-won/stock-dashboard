...
- **리더보드 추이 라인차트**: 총평가금액을 **평일 장마감 15:30(KST)에 하루 한 점(종가)** 적재
  (`@nestjs/schedule` 크론 `SNAPSHOT_CRON`, 재시작·중복 실행 시 그날 이미 있으면 skip). 차트 축은
  **오늘 기준 고정 30일**(`HISTORY_WINDOW_DAYS`) — 시즌은 어떤 스냅샷을 읽을지 **범위만** 정하고
  축엔 관여하지 않는다. **데이터 없는 앞 구간·늦게 들어온 참가자는 시드(100만·0%)로 백필**(0 으로
  채우면 y축 scale 이 무너지고 가짜 급등이 생김)해서 첫 스냅샷만 있어도 30점이라 곡선이 바로
  그려진다. 중간에 빠진 날(주말·휴장)은 직전 값 이월(`buildDailyHistory`). 시즌 초반엔 왼쪽이
  대부분 평평한 100만 baseline 이고 날이 갈수록 채워진다. **과거는 소급 복원 불가**(과거 시세를
  저장하지 않음). 스냅샷이 하나도 없을 때만 곡선 대신 안내 문구를 띄운다.
